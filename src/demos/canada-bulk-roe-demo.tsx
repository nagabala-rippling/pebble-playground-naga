          ...
          <Card.Layout padding={Card.Layout.PADDINGS.PX_24}>
            <TableHeader theme={theme}>
              <Input.SegmentedControl
                ...
                value={segmentFilter}
                onChange={(value) => setSegmentFilter(value as 'all' | 'not_completed' | 'completed')}
                list={[
            ...
            <SearchFilterRow theme={theme}>
              ...
              <FilterWrapper>
                <Input.Select
                  ...
                  onChange={(val: unknown) => setReasonFilter((val as string) || '')}
                  size={Input.Select.SIZES.S}
                  placeholder="Reason"
                  canClear
                />
                      ...
                      {employee.status !== 'Draft' || employee.hasErrors ? (
                        ...
                        >
                          <span style={{ display: 'inline-flex' }}>
                            <Input.Checkbox
                              label=""
                              name={`select-${employee.id}-disabled`}
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'center', marginTop: '-8px' }}>
          <Calendar
            ...
            onClick={(date?: Date | (Date | undefined)[]) => {
              if (date instanceof Date) {
                setApprovalSubmissionDate(date);
              } else if (Array.isArray(date) && date[0]) {
                setApprovalSubmissionDate(date[0]);
              }
            }}
